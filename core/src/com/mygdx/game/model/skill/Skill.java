...
public class Skill {
    ...
    public static Skill of(SkillType skillType, CreatureId creatureId) {
        if (skillType == SkillType.SWORD_SLASH) {
            return Skill.of(skillType, creatureId, singleScheduledAbility(AbilityType.SLASH, skillType),
                            SimpleTimer.getExpiredTimer(), 0.6f, 20f, 0f);
        }
        if (skillType == SkillType.FIREBALL) {
            return Skill.of(skillType, creatureId, singleScheduledAbility(AbilityType.FIREBALL, skillType),
                            SimpleTimer.getExpiredTimer(), 1f, 30f, 20f);
        }
        if (skillType == SkillType.LIGHTNING) {
            return Skill.of(skillType, creatureId, singleScheduledAbility(AbilityType.LIGHTNING_SPARK, skillType),
                            SimpleTimer.getExpiredTimer(), 1f, 20f, 26f);
        }
        if (skillType == SkillType.CROSSBOW_BOLT) {
            ScheduledAbility[] sequentialCrossbowBolts = {ScheduledAbility.of(AbilityType.CROSSBOW_BOLT, skillType, 0f),
            ...
            return Skill.of(skillType, creatureId, Stream.of(sequentialCrossbowBolts)
                    .collect(Collectors.toCollection(ArrayList::new)), SimpleTimer.getExpiredTimer(), 2f, 40f, 0f);
        }
        if (skillType == SkillType.MAGIC_ORB) {
            return Skill.of(skillType, creatureId, singleScheduledAbility(AbilityType.MAGIC_ORB, skillType),
                            SimpleTimer.getExpiredTimer(), 0.8f, 15f, 10f);
        }
        if (skillType == SkillType.SLOW_MAGIC_ORB) {
            return Skill.of(skillType, creatureId, singleScheduledAbility(AbilityType.MAGIC_ORB, skillType),
                            SimpleTimer.getExpiredTimer(), 1.3f, 15f, 10f);
        }
        if (skillType == SkillType.VOLATILE_BUBBLE) {
            return Skill.of(skillType, creatureId, singleScheduledAbility(AbilityType.VOLATILE_BUBBLE, skillType),
                            SimpleTimer.getExpiredTimer(), 1.3f, 15f, 20f);
        }
        if (skillType == SkillType.SUMMON_GHOSTS) {
            return Skill.of(skillType, creatureId, singleScheduledAbility(AbilityType.SUMMON_GHOSTS, skillType),
                            SimpleTimer.getExpiredTimer(), 1.3f, 15f, 20f);
        }

        if (skillType == SkillType.RICOCHET_BALLISTA) {
            return Skill.of(skillType, creatureId, singleScheduledAbility(AbilityType.RICOCHET_BALLISTA, skillType),
                            SimpleTimer.getExpiredTimer(), 1.3f, 15f, 20f);
        }

        if (skillType == SkillType.BOOMERANG) {
            return Skill.of(skillType, creatureId, singleScheduledAbility(AbilityType.BOOMERANG, skillType),
                            SimpleTimer.getExpiredTimer(), 6f, 30f, 0f);
        }

        if (skillType == SkillType.SUMMON_SHIELD) {
            return Skill.of(skillType, creatureId, singleScheduledAbility(AbilityType.SUMMON_SHIELD, skillType),
                            SimpleTimer.getExpiredTimer(), 2f, 25f, 0f);
        }
        if (skillType == SkillType.SWORD_SPIN) {
            return Skill.of(skillType, creatureId, singleScheduledAbility(AbilityType.SWORD_SPIN, skillType),
                            SimpleTimer.getExpiredTimer(), 4f, 30f, 0f);
        }
        if (skillType == SkillType.TELEPORT) {
            return Skill.of(skillType, creatureId, singleScheduledAbility(AbilityType.TELEPORT, skillType),
                            SimpleTimer.getExpiredTimer(), 2f, 10f, 35f);
        }
