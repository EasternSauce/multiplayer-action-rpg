...
public class PhysicsWorld {
    ...
    public Vector2 getTileCenter(Vector2Int pos) {
        ...
        return Vector2.of(
            pos.getX() * tileWidth + tileWidth / 2,
            pos.getY() * tileHeight + tileHeight / 2
        );
    }

    public Vector2Int getClosestTile(Vector2 pos) {
        return Vector2Int.of(
            (int) (pos.getX() / tileWidth),
            (int) (pos.getY() / tileHeight)
        );
    }
    ...
    public void createTerrainTiles() {
        for (int layerNum = 0; layerNum <= 1; layerNum++) {
                ...
                for (int x = 0; x < layer.getWidth(); x++) {
                    ...
                    if (cell != null) {
                        ...
                        if (!isTileTraversable) {
                            ...
                            List<Vector2Int> combinations = Arrays.asList(
                                Vector2Int.of(
                                    0,
                                    1
                                ),
                            ...
                            combinations.stream().filter(pos -> tileExists(
                                _x + pos.getX(),
                                _y + pos.getY()
                            )).forEach(tilePos -> traversablesWithMargins.put(
                                Vector2Int.of(
                                    _x + tilePos.getX(),
                                    _y + tilePos.getY()
                                ),
            ...
            for (int y = 0; y < layer.getHeight(); y++) {
                for (int x = 0; x < layer.getWidth(); x++) {
                    if (!traversables.get(Vector2Int.of(
                        x,
                        y
                    ))) {
                        TerrainTileBody tile = TerrainTileBody.of(
                            Vector2Int.of(
                                x,
                                y
                            ),
    ...
    public void createBorders() {
        ...
        for (int x = 0; x < widthInTiles(); x++) {
            TerrainTileBody tile1 = TerrainTileBody.of(
                Vector2Int.of(
                    x,
                    -1
                ),
            ...
            TerrainTileBody tile2 = TerrainTileBody.of(
                Vector2Int.of(
                    x,
                    heightInTiles()
                ),
