...
public class Astar {
    public static Map<Vector2Int, PathingNode> generatePathingGraph(PhysicsWorld world) {
        ...
        for (int y = 0; y < world.heightInTiles(); y++) {
            for (int x = 0; x < world.widthInTiles(); x++) {
                ...
                if (x + 1 < world.widthInTiles() && y - 1 >= 0 && world.getTraversables().get(Vector2Int.of(x + 1,
                    y
                )) && world.getTraversables().get(Vector2Int.of(x, y - 1))) {
                    tryAddEdge(pathingNodes, world, Vector2Int.of(x, y), Vector2Int.of(x + 1, y - 1), diagonalWeight);
                }

                if (x - 1 >= 0 && y + 1 < world.heightInTiles() && world.getTraversables().get(Vector2Int.of(x - 1,
                    y
                )) && world.getTraversables().get(Vector2Int.of(x, y + 1))) {
                    tryAddEdge(pathingNodes, world, Vector2Int.of(x, y), Vector2Int.of(x - 1, y + 1), diagonalWeight);
                }

                if (x + 1 < world.widthInTiles() && y + 1 < world.heightInTiles() && world.getTraversables().get(
                    Vector2Int.of(x + 1, y)) && world.getTraversables().get(Vector2Int.of(x, y + 1))) {
                    tryAddEdge(pathingNodes, world, Vector2Int.of(x, y), Vector2Int.of(x + 1, y + 1), diagonalWeight);
                }
            }
        }
    ...
    public static List<Vector2Int> reconstructPath(AstarNode lastNode, AstarState result) {
        if (lastNode.getParent() != null) {
            List<Vector2Int> list = new LinkedList<>(reconstructPath(result.getAstarGraph().get(lastNode.getParent()),
                result
            ));
    ...
    public static AstarState processNeighbor(AstarState astarState,
                                             ...
                                             PhysicsWorld world,
                                             Integer capability) {
        if (astarState.getClosedSet().contains(pathingEdge.getNeighborPos()) || Astar.calculateHeuristic(originNodePos,
            astarState.getFinishPos()
        ) >= 60 &&
    ...
    public static Map<Vector2Int, AstarNode> getAstarGraph(Map<Vector2Int, PathingNode> pathingGraph) {
        return pathingGraph.entrySet().stream().collect(Collectors.toMap(Map.Entry::getKey,
