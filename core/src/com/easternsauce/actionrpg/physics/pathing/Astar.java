...
public class Astar {
    public static Map<Vector2Int, PathingNode> generatePathingGraph(PhysicsWorld world) {
            ...
            for (int x = 0; x < world.widthInTiles(); x++) {
                pathingNodes.put(
                    ...
                    PathingNode.of(
                        ...
                        world.getClearances().getOrDefault(
                            Vector2Int.of(
                                x,
                                y
                            ),
        ...
        for (int y = 0; y < world.heightInTiles(); y++) {
            for (int x = 0; x < world.widthInTiles(); x++) {
                ...
                if (x - 1 >= 0 && y - 1 >= 0 && world.getTraversables().get(Vector2Int.of(
                    x - 1,
                    y
                )) && world.getTraversables().get(Vector2Int.of(
                    x,
                    y - 1
                ))) {
                    tryAddEdge(
                ...
                if (x + 1 < world.widthInTiles() && y - 1 >= 0 && world.getTraversables().get(Vector2Int.of(
                    x + 1,
                    y
                )) && world.getTraversables().get(Vector2Int.of(
                    x,
                ...
                if (x - 1 >= 0 && y + 1 < world.heightInTiles() && world.getTraversables().get(Vector2Int.of(
                    x - 1,
                    y
                )) && world.getTraversables().get(Vector2Int.of(
                    x,
    ...
    public static void tryAddEdge(
        ...
        if (0 <= toPos.getY() &&
            ...
            if (world.getTraversables().get(fromPos) && world.getTraversables().get(toPos)) {
                ...
                pathingNodes.put(
                    fromPos,
                    pathingNodes.get(fromPos).withEdge(
                        weight,
                        targetNode
                    )
                );
    ...
    public static AstarState traverse(
        ...
        while (!astarState.getIsGaveUp() && !astarState.getOpenSet().isEmpty() && !astarState.getFoundPath()) {
            ...
            Vector2Int minimumTile = Collections.min(
                ...
                (o1, o2) -> {
                    if (Objects.equals(
                        finalAstarState.getAstarGraph().get(o1).getF(),
            ...
            AstarState resultingAstarState = AstarState.of(
                astarState.getAstarGraph(),
                astarState.getOpenSet(),
            ...
            if (currentNode.getPos().equals(finishTilePos)) {
                ...
                for (int i = 0; i < currentNode.getPathingNode().getOutgoingEdges().size(); i++) {
                    ...
                    resultingAstarState = processNeighbor(
                        resultingAstarState,
                        currentNode.getPos(),
                        pathingEdge,
    ...
    public static AstarState processNeighbor(
        ...
        if (astarState.getClosedSet().contains(pathingEdge.getNeighborPos()) || Astar.calculateHeuristic(
            originNodePos,
            astarState.getFinishPos()
        ) >= 60 && world.getClearances().get(pathingEdge.getNeighborPos()) < capability) {
            return astarState;
        }
        ...
        AstarNode updatedNode = AstarNode.of(
            neighborNode.getPathingNode(),
        ...
        if (!astarState.getOpenSet().contains(neighborNode.getPos())) {
            updatedNode.setH(Astar.calculateHeuristic(
                neighborNode.getPos(),
                astarState.getFinishPos()
            ));
            ...
            AstarState updatedAstarState = AstarState.of(
                astarState.getAstarGraph(),
                astarState.getOpenSet(),
                astarState.getClosedSet(),
                astarState.getFinishPos(),
    ...
    public static AstarResult findPath(
        ...
        AstarState astarState = AstarState.of(
            freshAstarGraph,
    ...
    public static Map<Vector2Int, AstarNode> getAstarGraph(Map<Vector2Int, PathingNode> pathingGraph) {
        return pathingGraph.entrySet().stream().collect(Collectors.toMap(
            Map.Entry::getKey,
            stuff -> AstarNode.of(stuff.getValue())
        ));
    }
}
