  ...
  public void render(Creature creature, RenderingLayer renderingLayer) {
    if (creature != null && !creature.isAlive()) {
      if (creature.getParams().getTimeSinceDeathTimer().getTime() < creature.getParams().getRespawnTime()) {
        float timeRemainingBeforeRespawn = creature.getParams().getRespawnTime() - creature.getParams().getTimeSinceDeathTimer().getTime();
        String timeRemainingBeforeRespawnText = String.format(Locale.US, "%.2f", timeRemainingBeforeRespawn);

        Assets.renderVeryLargeFont(renderingLayer, "You are dead!\nRespawning...\n" + timeRemainingBeforeRespawnText, Vector2.of(Constants.WINDOW_WIDTH / 2f - Constants.WINDOW_WIDTH / 8f, Constants.WINDOW_HEIGHT / 2f + Constants.WINDOW_HEIGHT / 5f), Color.RED);
      }
    }
  }
}
