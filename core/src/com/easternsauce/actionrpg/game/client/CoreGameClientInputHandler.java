...
public class CoreGameClientInputHandler {
    ...
    private void handleChatMessageActionInput(CoreGameClient game) {
        if (!game.getChat().getTyping()) {
            ...
            if (!game.getChat().getCurrentMessage().isEmpty()) {
                game.getEndPoint().sendTCP(ChatMessageSendCommand.of(game
                    .getGameState()
                    .getThisClientPlayerId()
                    .getValue(), game.getChat().getCurrentMessage()));

                game.getChat().sendMessage(game.getGameState().getThisClientPlayerId().getValue(),
        ...
        if (game.getChat().getTyping()) {
            if (game.getChat().getHoldingBackspace()) {
                if (!game.getChat().getCurrentMessage().isEmpty() &&
                    ...
                    game.getChat().setCurrentMessage(game
                        .getChat()
                        .getCurrentMessage()
                        .substring(0, game.getChat().getCurrentMessage().length() - 1));
                }
            } else {
    ...
    private void handleDebugInformationQueryInput(CoreGameClient game) {
        CreatureId creatureId = game.getActiveCreatures().keySet().stream().filter(cId -> cId
            .getValue()
    ...
    private void handleAttackButtonHoldInput(PlayerConfig playerConfig, CoreGameClient game) {
        if (!playerConfig.getInventoryVisible()) {
            ...
            game.getEndPoint().sendTCP(ActionPerformCommand.of(SkillTryPerformAction.of(game
                    .getGameState()
                    .getThisClientPlayerId(),
                attackSkill,
    ...
    private void handleUsePotionMenuItemInput(int potionMenuItemIndex, CoreGameClient game) {
        game.getEndPoint().sendTCP(ActionPerformCommand.of(PotionMenuItemUseAction.of(game
            .getGameState()
            .getThisClientPlayerId(), potionMenuItemIndex)));

    }

    private void handleInventoryWindowActionInput(CoreGameClient game) {
        game.getEndPoint().sendTCP(ActionPerformCommand.of(InventoryWindowToggleAction.of(game
            .getGameState()
            .getThisClientPlayerId())));
    }
}
